...
public:
    ...
    const std::vector<std::string> all(const std::string& d) const
    {
        ...
        return (_append_map.find(d) == _append_map.end()) ? empty
                                                          : _append_map.at(d);
    }
...
namespace detail {
    ...
    public:
        ...
        explicit str_wrap(const std::string& w)
            : lwrap(w)
            , rwrap(w)
        {
        }
    ...
    static std::string& str_replace_helper(std::string& s,
    ...
    static std::string str_replace(const std::string& s,
        const std::string& patt,
        const std::string& repl)
    {
        std::string tmp = s;
    ...
    static std::string str_format(const std::string& s,
        size_t pre,
        size_t len,
...
public:
    ...
    {
        _action = a;
        if (a == "store_const" or a == "store_true" or a == "store_false" or a == "append_const" or a == "count" or a == "help" or a == "version") {
            nargs(0);
        }
        return *this;
    }
...
private:
    std::string check_type(const std::string& opt, const std::string& val) const
    {
        ...
        if (type() == "int" or type() == "long") {
            long t;
            if (not(ss >> t)) {
                err << "option" << " " << opt << ": " << "invalid integer value"
                    << ": '" << val << "'";
            }
            ...
            if (not(ss >> t)) {
                err << "option" << " " << opt << ": " << "invalid complex value"
                    << ": '" << val << "'";
            }
        }
    ...
    std::string format_help(const unsigned int indent = 2) const
    {
        ...
        if (not help().empty()) {
            std::string help_str = (not get_default().empty())
...
public:
    ...
    Option& add_option(const std::vector<std::string>& opt)
    {
        ...
        for (std::vector<std::string>::const_iterator it = opt.begin();
            it != opt.end(); ++it) {
            if (it->substr(0, 2) == "--") {
    ...
    Option& add_option(const std::string& opt1,
    ...
    virtual void exit(int code) const { std::exit(code); }
...
private:
    ...
    void process_opt(const Option& o,
        const std::string& opt,
...
public:
    OptionParserExcept() { }
    OptionParserExcept(const OptionParser& parser)
        : OptionParser(parser)
    {
    }
    virtual void exit(int code) const { throw code; }
};
...
namespace detail {
        ...
        const std::vector<std::string>& v)
    {
        ...
        if (parser.add_version_option() and not parser.version().empty()) {
            ...
            parser._opts.splice(parser._opts.begin(), parser._opts,
                --(parser._opts.end()));
        }
        ...
        for (std::list<Option>::const_iterator it = parser._opts.begin();
            it != parser._opts.end(); ++it) {
    ...
    static std::string format_help_helper(const OptionParser& parser)
    {
        ...
        if (not parser.epilog().empty()) {
            ss << std::endl
               << detail::str_format(parser.epilog(), 0, detail::cols());
        }
